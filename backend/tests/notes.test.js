...
const authHeader = () => {
  const token = jwt.sign(
    { sub: AUTH_USER_ID, email: "ada@example.com" },
    env.jwtSecret,
    {
      expiresIn: "1h",
    },
...
describe("PUT /api/notes/:id", () => {
  ...
  it("passes only the provided field through on a title-only update, leaving content undefined for COALESCE to preserve", async () => {
    ...
    expect(
      updateStub.calledWith(1, AUTH_USER_ID, {
        title: "Renamed",
        content: undefined,
      }),
    ).to.be.true;
  ...
  it("surfaces repository update errors with note-specific context", async () => {
    const repositoryError = new Error("db down");
    sinon.stub(pool, "query").rejects(repositoryError);

    try {
      await notesRepository.updateForUser(1, AUTH_USER_ID, {
        title: "Renamed",
      });
      throw new Error("Expected updateForUser to reject");
    } catch (err) {
      expect(err.message).to.include("Failed to update note");
